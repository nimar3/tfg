...
A continuación como ejemplo la representación de un \gls{equipo} (\foreign{english}{Team}):
...
Además, otra buena práctica es separar las aplicaciones en módulos para añadir extensibilidad mediante reglas, nuevas vistas, separación de roles y otros beneficios añadidos mediante el uso de \foreign{english}{blueprints} \cite{book-mastering-flask}, una funcionalidad incluida en \textit{Flask}.
...
Por último, otro beneficio que brinda esta separación de roles es que se pueden ejecutar un número indefinido de \foreign{english}{Workers}, donde cada uno puede tener diferentes colas configuradas y unos recursos disponibles exclusivos para cada uno de ellos.

\chapter{Desarrollo de la solución}

En este capítulo presentamos un resumen de como se ha pasado del diseño de la aplicación a la solución final; no se explicará el desarrollo del código fuente en si, sino un repaso de ciertas particularidades de la aplicación como el \gls{envio} de \gls{tarea}s, los \gls{paquete}s que permiten todas las funcionalidades disponibles, el uso de un \foreign{english}{boilerplate}, una plantilla que simplificará el desarrollo de la interfaz web, el uso de la librería \textit{RQ} para las colas internas y un resumen de los entregables que se fueron produciendo para dar vida al producto final.

\section{Flask \foreign{english}{boilerplate}}

La usabilidad es uno de los aspectos más importantes de una aplicación web y en este caso no se puede dejar de lado. Un \textit{usuario} de \textit{Pizarra} esperará una interfaz intuitiva y fácil de utilizar que le facilite el proceso de envío de soluciones (\foreign{english}{Requests}) a los problemas propuestos (\foreign{english}{Assignments}); No queremos que la aplicación se interponga entre el proceso de aprendizaje y la satisfacción de participar y obtener logros \foreign{english}{Badges}. Una gran cantidad de aplicaciones utilizan las mismas librerías para maquetar los sitios web, entre estas librerías la más importante es \textit{Bootstrap}\footnote{Sitio web Bootstrap: \url{https://getbootstrap.com/}}, y además, a partir de estas librerías existen \foreign{english}{Dashboards} o Paneles de Control de código abierto para utilizar como punto de partida en nuevas aplicaciones.
...
Este conjunto de librerías y plantillas HTML tienen el nombre de \foreign{english}{boilerplate} y aceleran el desarrollo de una aplicación web a usuarios noveles como también expertos ya que solo deben preocuparse de reutilizar componentes ya predefinidos.
...
Un sistema de administración de paquetes es un \foreign{english}{software} que se encarga de resolver las referencias en un repositorio central, su descarga e instalación de forma segura.
...
En el siguiente ejemplo podemos ver la creación de un ``trabajador'' o \foreign{english}{Worker} al iniciar \textit{Pizarra} en éste modo. 
...
\begin{enumerate}
	...
	\item pone al \foreign{english}{Worker} a ``trabajar'' revisando el servicio de \textit{Redis} periodicamente para ver si hay \foreign{english}{jobs} a ejecutar.
\end{enumerate}
...
El método \textit{create\_job} recibe como parámetro un \textit{Request}, donde su definición no es relevante salvo que se utilizará para el \textit{job} a procesar y además tiene el nombre de la \foreign{english}{Queue} interna donde debe encolarse. Lo que hace es lo siguiente:
...
Este autómata tiene una serie de etiquetas que ayudarán a saber a que estado pasar a continuación (\foreign{english}{StepResult}). Al crear un objeto del tipo \textit{LocalJob} en esta versión simplificada se puede ver como se asigna el \foreign{english}{Request} y además existe la definición del autómata como un mapa, donde para una etiqueta se le asigna un método a ejecutar y un mapa de transiciones dependiendo el resultado de la ejecución del paso. \newline
...
En el tercer bloque se encuentra la función \textit{process}, que como explicamos previamente es el método que ejecuta el \foreign{english}{Worker} al obtener un \foreign{english}{job} de una \gls{cola}. Lo que hace es obtener el estado actual de la \foreign{english}{Request} y hacer el siguiente proceso hasta terminar:
...
Cuando se llegó a la etapa de desarrollo en el cual se tenían que crear las \gls{insignia}s (\foreign{english}{Badges}) se encontró un problema de gran dificultad. ¿Cómo hacer que un \gls{administrador} desde su panel web pueda hacer que una \gls{insignia} se aplique cuando suceda algo en particular? Las \gls{insignia}s no solo se obtienen al completar una \gls{tarea} de forma correcta, sino que también en otros casos que puedan darse; como por ejemplo, que el \gls{envio} se encuentre en los primeros tres puestos de la tabla de posiciones (\foreign{english}{leaderboard}).
...
En el ejemplo previose tenemos la creación de dos \gls{insignia}s que se asignarán a un \gls{alumno} cuando la ejecución de su programa sea en menos de 10 segundos y también otro caso donde la posición de la ejecución quede entre las tres mejores. La descripción de los atributos es:
