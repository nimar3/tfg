...
Al crear una nueva aplicación en \textit{Flask}, existen ciertos patrones o mejores prácticas a seguir. Entre ellas se encuentra la utilización de uno de los patrones de diseño más comunes en la ingeniería de software, \acrlong{mvc} (\acrshort{mvc}). Con este patrón se crea una separación de capas entre lo que es el Modelo, que contiene la representación de los datos y su persistencia, la Vista, que es la interfaz de usuario, y el Controlador, como intermediario entre el Modelo y la Vista.
...
Para poder utilizar las \Gls{cola}s de \kahan es necesario poder conectarnos vía protocolos seguros, ya sea para ejecutar comandos o para copiar ficheros.
...
Para facilitar el desarrollo y su posterior mantenimiento se debe poder disponer de una forma fácil y estandarizada de carga de datos de ejemplo. Para esta funcionalidad se ha optado por ficheros en formato \textit{yaml}.
...
Pizarra es una herramienta que necesita de otros aplicativos para su funcionamiento: una \textit{BD} para la persistencia de datos, Redis para el sistema de colas interno de envío de tareas, un \acrshort{nfs} para compartir recursos y Nginx para actuar como servidor web y redirigir las peticiones. 

En el pasado, la implantación hubiera requerido la intervención del equipo de Administración de Sistemas que se encargue de instalar el \textit{SO} de cada máquina, crear usuarios, asignar permisos, abrir puertos y otro sinfín de tareas. Hoy en día es diferente.
...
Docker ofrece una funcionalidad similar a la de una máquina virtual, siendo algo bastante diferente. Docker posee algunas ventajas con respecto a las máquinas virtuales, como son la asignación de recursos de forma dinámica o la necesidad de un espacio en disco reducido, ya que podemos evitar la instalación de un \acrshort{so} en los contenedores y la portabilidad (Figura~\ref{figura:container-vs-vm}).
...
Para nuestro entorno de desarrollo local utilizamos contenedores públicos como el de Postgres que, con un fichero de configuración, permite tener el servicio ejecutándose en cuestión de minutos.

A continuación, se muestra un extracto del fichero \textbf{docker-compose.yaml} del repositorio\footnote{Docker yaml: \url{https://github.com/nimar3/pizarra/blob/master/docker-compose.yml}.}: 
\newline
...
\begin{itemize}
	...
	\item \textbf{volumes}: mapeo de directorios de nuestro entorno de desarrollo local al contenedor. Esto se lleva a cabo para evitar perder la información de la \textit{BD} al ser los contenedores \foreignlanguage{english}{stateless}.
    ...
    \item \textbf{environment}: variables de entorno del \textit{SO}, en nuestro caso definimos un usuario y una contraseña para acceder a la \textit{BD}.
...
Existe ciertas limitaciones en \foreign{english}{cluster} \kahan que han impedido la instalación del servicio de Kubernetes. Este era uno de los riesgos contemplados en el trabajo. Para resolver este problema se ha optado por un despliegue de Pizarra en la nube. 

Existen varios proveedores de \textit{K8s} en Internet (puede consultarse el listado de todos los \foreign{english}{partners} en el sitio oficial de Kubernetes\footnote{Kubernetes - Partners: \url{https://kubernetes.io/es/partners}.}) siendo los más importantes: Google Cloud Engine (\textit{GCE}), Amazon Web Services (\textit{AWS}) y Microsoft Azure. Se ha optado por desplegar la aplicación en \textit{GCE}, ya que dispone de buena documentación y, además, \textdollar 300 de crédito promocional, que es más que suficiente para tener el aplicativo corriendo durante meses.
...
Con nuestra cuenta accedemos a la Consola Web de \acrshort{gce} y creamos nuestro proyecto, en este caso lo llamaremos Pizarra (Figura~\ref{figura:gce-new-project}). Nuestro aplicativo utiliza Kubernetes Engine y el Container Registry que son visibles desde el menú lateral. Entraremos a cada uno de ellos para habilitar su \acrshort{api}.
...
Las variables de entorno necesarias son el ID del proyecto en \acrshort{gce} y las credenciales de la \acrshort{bd} que serán guardados como un secreto en Kubernetes. Podemos exportarlas antes de lanzar nuestro script ejecutando los siguientes comandos en la consola:
...
En total llegamos a un costo estimado de \texteuro 49.00 mensuales. Como dato adicional, se puede variar de Centro de Datos y otros parámetros como el uso estipulado para reducir los costos aún más. Podemos guardar la estimación o enviárnosla por correo electrónico (Figura~\ref{figura:gce-estimated-cost-email}).
...
Este ha sido un trabajo de una sola persona y se ha seguido el método de \foreign{english}{Feature Branch Workflow} \cite{link-git-flow} donde se tiene una rama principal y nuevas funcionalidades o \foreign{english}{features} divergen de esta para luego volver a fusionarse. Ya que se espera recibir colaboraciones a futuro se deberá acomodar a un proceso que involucre a un equipo de trabajo y ramas en diferentes estados como \foreign{english}{dev, release} y \foreign{english}{master} como en \foreign{english}{Gitflow Workflow}.
